...
class OpenSSLConan(ConanFile):
    ...
    source_tgz = "https://www.openssl.org/source/openssl-%s.tar.gz" % short_version
    source_tgz_old = "https://www.openssl.org/source/old/1.0.2/openssl-%s.tar.gz" % short_version
    ...
    @property
    def subfolder(self):
        return "openssl-%s" % self.short_version
    ...
    def unix_build(self, config_options_string):
        ...
        if self.settings.os == "Linux":
            if arch == "x86":
                ...
                if arch.startswith('armv') and int(arch[4]) < 4:
                    raise Exception('Unsupported ARM arch on Linux (%s). Only armv4 and newer supported.' % arch)
                target = "%slinux-armv4" % target_prefix
            else:
                raise Exception('Unsupported arch on Linux: %s' % arch)
            ...
            if arch in ["sparc", "x86"]:
                target = "%ssolaris-%s%s" % (target_prefix, arch, suffix)
            elif arch in ["sparcv9", "x86_64"]:
                target = "%ssolaris64-%s%s" % (target_prefix, arch, suffix)
            else:
                raise Exception("Unsupported arch on SunOS: %s" % arch)

        elif self.settings.os == "FreeBSD":
            target = "%sBSD-%s" % (target_prefix, arch)
        ...
        self.run("make -C %s -j%d" % (self.subfolder, multiprocessing.cpu_count()))
    ...
    def ios_build(self, config_options_string):
        ...
        tools.replace_in_file("./openssl-%s/Makefile.shared" % self.short_version, old_str, new_str)
        self.output.warn("----------MAKE OPENSSL %s-------------" % self.short_version)
    ...
    def visual_build(self, config_options_string):
        ...
        self.output.warn("----------CONFIGURING OPENSSL FOR WINDOWS. %s-------------" % self.short_version)
        ...
        replace_runtime_in_file("./openssl-%s/ms/ntdll.mak" % self.short_version)
        replace_runtime_in_file("./openssl-%s/ms/nt.mak" % self.short_version)
